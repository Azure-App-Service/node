...
docker build -t "$1"/node:4.5.0-2 -t "$1"/node:4.5-2 -t 4.5
...
docker build -t "$1"/node:8.0.0 -t "$1"/node:8.0 8.0
docker build -t "$1"/node:8.1.2 -t "$1"/node:8.1 -t "$1"/node:8 -t "$1"/node:latest 8.1
